...
/*const nveggie= bonde.filter((pessoa)=> pessoa.vegetariana ==! true);
console.log(nveggie.length);
const veggie = bonde.filter((pessoa) => pessoa.vegetariana ==  true);
console.log(veggie.length);
const reais= bonde.filter((pessoa)=> pessoa.saldoNaConta >50);
console.log(reais);*/

bonde.forEach((pessoa, index) => console.log(`${pessoa.apelido} ${pessoa.idade}`));
//for(let i=0; i<bonde.length; i++){ console.log(`${bonde[i].apelido} ${bonde[i].idade}`)};

/*const bondeFriends= bonde.map((pessoas)=> {
    pessoas.seriesPreferidas.find((e)=> e ==="Friends";)
})
console.log(bondeFriends);*/
//const numerosDuplicados = numeros.map(e=> e*2);
//console.log(numerosDuplicados);
const riqueza=[2,3,4,5,6,7];
console.log(
    bond.reduce((valoranterior, valoratual) => valoranterior + valoratual, 0);)
